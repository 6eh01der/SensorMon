...
void GetSensors()
{
  for(int i = 0; i < 2; i ++ )                           // Проходим по сенсорам
  {
...
///////////////////////// 2  Выход клапан воды                           
    ...
    for(int i = 0; i < 2; i ++ )
    {
      sensor[i].adress_ = i;                                    // Инициализируем адрес зоны
      switch (i)                                              // Инициализируем пины тревоги и уведомления о сработках
      {
      case 0:                                                 // Для первого датчика
       strcpy(sensor[i].message_alarm_,"Triggered to sensor 1!"); // Формируем уведомление о сработке
       break;
      case 1:                                                 // Для второго датчика
       strcpy(sensor[i].message_alarm_,"Triggered to sensor 2!"); // Формируем уведомление о сработке
       break;
      }
    }
  }
