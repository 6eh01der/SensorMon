...
void setup() 
{
  ...
  sensor.adress_ = 4;                                    // Инициализируем адрес датчика
  strcpy(sensor.message_alarm_,"No air flow!!!");       // Формируем уведомление о сработке 
  sensor.status_ = 1;
...
void GetSensors()
{
  if(sensor.ReadPin())                                // Если есть сработка
  {
...
void AlarmMessages()
{
  if(flag_alarm)
  {
    ...
    {
      if(sensor[4].alarm_ == true && sensor[4].send_alarm_ == false) // Если поднят флаг сработки датчика и уведомление не отправлялось
      {
          SendSMS(sensor[4].message_alarm_);                                 // Отправляем смс уведомление
          sensor[4].send_alarm_ = true;                                      // Поднимаем флаг отправленного уведомления
  ...
  if (result.length()  ==  1)                     // Если в сообщении одна цифра
  {
    ...
    {                                             // Если все нормально, исполняем команду
      if (ledState == 1) 
      {
        ...
        sensor.status_ = 1;                          // Инициализируем макрос
...
void TestModem()
{
  do {              // При включении питания МК загрузится раньше сим модуля поэтому ждем загрузки и ответа на команду
    ...
    if(AT_counter == 3)
    {
      D13_High;                                 // Сигнализируем звуком об отсутсвии связи с сим-модулем
      delay(150);
      D13_Low;
      delay(20);
      D13_High;                  
